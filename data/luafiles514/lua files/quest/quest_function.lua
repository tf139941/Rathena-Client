...
QuestTable["func_quest_lower_exchange_imagetag"] = function(msg)
	local imagetag_front_startpos = string.find(msg, "<%s*image%s*=%s*\"%d*\"%s*>")
	if nil ~= imagetag_front_startpos and nil ~= msg then
		local imagetag_front = string.sub(msg, imagetag_front_startpos, msg)
		local imagetag_rear_startpos = string.find(msg, "<%s*\\end%s*>", msg + 1)
		if nil ~= imagetag_rear_startpos and nil ~= msg then
			local imagetag_rear = string.sub(msg, imagetag_rear_startpos, msg)
			local name = string.sub(msg, msg + 1, imagetag_rear_startpos - 1)
			local num_startpos = string.find(imagetag_front, "\"%d*\"")
			local num_endpos = string.sub(msg, imagetag_front_startpos, msg)
			if nil ~= num_startpos and nil ~= num_endpos then
...
QuestTable["func_quest_lower_exchange_linktag"] = function(msg)
	local linktag_front_startpos = string.find(msg, "<%s*link%s*=%s*\".*\">")
	if nil ~= linktag_front_startpos and nil ~= msg then
		local linktag_front = string.sub(msg, linktag_front_startpos, msg)
		local linktag_rear_startpos = string.find(msg, "<%s*\\end%s*>", msg + 1)
		if nil ~= linktag_rear_startpos and nil ~= msg then
			local linktag_rear = string.sub(msg, linktag_rear_startpos, msg)
			local name = string.sub(msg, msg + 1, linktag_rear_startpos - 1)
			local spritename_startpos = string.find(linktag_front, "\".*\"")
			local spritename_endpos = string.sub(msg, linktag_front_startpos, msg)
			if nil ~= spritename_startpos and nil ~= spritename_endpos then
				...
				local final = ""
				if 1 < linktag_front_startpos then
...
query_quest_tab_episode = function(tabname, episodeseqId)
	if nil == QuestTable[tabname] then
		return
	elseif nil == QuestTable[tabname][episodeseqId] then
		return
	end
	local episode = QuestTable[tabname][episodeseqId]
	return QuestTable[tabname][episodeseqId].name, episode.imagefile
end
...
query_quest_tab_chapter = function(tabname, episodeseqId, chapterseqId)
	...
	elseif nil == QuestTable[tabname][episodeseqId] then
		return
	elseif nil == QuestTable[tabname][episodeseqId][chapterseqId] then
		return
	end
	local chapter = QuestTable[tabname][episodeseqId][chapterseqId]
	return QuestTable[tabname][episodeseqId][chapterseqId].name
end
...
query_quest_tab_summary = function(tabname, episodeseqId, chapterseqId, questseqId)
	...
	elseif nil == QuestTable[tabname][episodeseqId][chapterseqId] then
		return
	elseif nil == QuestTable[tabname][episodeseqId][chapterseqId][questseqId] then
		return
	end
	local summary = QuestTable[tabname][episodeseqId][chapterseqId][questseqId]
	return QuestTable[tabname][episodeseqId][chapterseqId][questseqId].name, summary.questID
end
